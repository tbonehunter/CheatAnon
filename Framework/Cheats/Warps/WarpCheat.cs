...
internal class WarpCheat : BaseCheat
{
    ...
    *********/
    ...
    public override IEnumerable<OptionsElement> GetFields(CheatContext context)
    {
        ...
        bool enforceRestrictions = context.Config.EnforceWarpProgressionRestrictions;
        ...
        foreach ((string sectionKey, List<WarpContentModel> sectionWarps) in warps)
        {
            ...
            foreach (WarpContentModel warp in sectionWarps)
            {
                ...
                // skip warps that don't meet progression restrictions
                if (!WarpRestrictions.CanAccessLocation(warp.Location, enforceRestrictions))
                    continue;

                // special check for Quarry warp (it's on Mountain map)
                if (enforceRestrictions && WarpRestrictions.IsQuarryWarp(warp.Location, (int)warp.Tile.X, (int)warp.Tile.Y))
                {
                    if (!WarpRestrictions.CanAccessQuarry())
                        continue;
                }
    ...
    *********/
    ...
    private CheatsOptionsNumberWheel CreateMinesButton(WarpContentModel warp, int slotWidth, bool enforceRestrictions)
    {
        ...
        // If enforcing restrictions, limit max level to deepest reached
        int maxLevel = isSkullCavern ? 999_999 : bottomOfMine;
        if (enforceRestrictions && deepestLevel > 0)
        {
            maxLevel = isSkullCavern 
                ? Math.Min(deepestLevel + bottomOfMine, 999_999)
                : Math.Min(deepestLevel, bottomOfMine);
        }
